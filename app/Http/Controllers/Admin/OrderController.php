...
class OrderController extends Controller
{
    ...
    public function list(Request $request, $status)
    {
        ...
        $orders = $orders->whereIn('order_type', ['apps', 'default_type',])->latest()->paginate(500)->appends(['search' => $request['search'], 'from' => $request['from'], 'to' => $request['to']]);
        // dd($orders);
    ...
    public function edit($id)
    {
        ...
        if ($order->order_type == 'default_type' || $order->order_type == 'apps') {
            return view('admin-views.order.edit', compact('shipping_address', 'order', 'delivery_men', 'orderHistories', 'curiers'));
        } else {
            return view('admin-views.pos.order.edit', compact('order'));
        }
    }
    public function orderUpdate(Request $request, $id)
    {
        ...
        try {
            ...
            $order->update([
                'discount_amount' => BackEndHelper::currency_to_usd($request->discount_amount) ?? 0,
                'discount_type' => $request->discount_amount == 0 ? null : 'coupon_discount',
                'order_amount' => BackEndHelper::currency_to_usd($request->grand_total) ?? 0,
            ...
            if ($request->has('product_id')) {
                foreach ($request->product_id as $index => $productId) {
                    ...
                    $order->details()->create([
                        ...
                        'qty' => $qty,
                        'price' => $price,
                        'tax' => ($product->tax ?? 0) * $qty,
                        'discount' => ($product->discount ?? 0) * $qty,
                        'discount_type' => 'discount_on_product',
                        'variant' => $request->variant[$index] ?? null,
                        'variation' => $request->variation[$index] ?? null,
            ...
            Toastr::success(\App\CPU\translate('Order updated successfully!'));
            return redirect()->back();
        } catch (\Exception $e) {
            DB::rollBack();
            Toastr::error('Order update failed: ' . $e->getMessage());
            return redirect()->back();
        }
    }
    ...
    public function productSearch(Request $request)
    {
        ...
        $products = Product::where('name', 'like', "%$query%")
            ...
            ->get()
            ->map(function ($p) {
                $p->unit_price = \App\CPU\BackEndHelper::usd_to_currency($p->unit_price);
                return $p;
            });
        // dd($products);
