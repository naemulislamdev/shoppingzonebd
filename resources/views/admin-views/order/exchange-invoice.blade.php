...
<div class="first" style="display: block; height:auto !important;">
    <table class="">
        <tr>
            <th style="text-align:center">
                 <h1 style="color: #2D7BFF; margin-bottom: 0px; font-size: 30px;text-transform: capitalize">{{\App\CPU\translate('invoice')}}</h1>
    ...
    <!--<table class="bs-0">-->
    <!--    <tr>-->
    <!--        <th class="bg-primary content-position-y" style="padding-right: 0; height: 44px; text-align:right">-->
    <!--            <div>-->
            ...
            <tr>
                ...
                <td valign="top" >
                    ...
                    <div class="h4 montserrat-normal-600">
                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->shippingAddress ? $order->shippingAddress['contact_person_name'] : $order->customer['f_name'].' '.$order->customer['l_name']}}</p>
                        <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->shippingAddress ? $order->shippingAddress['phone'] : $order->customer['phone']}}</p>
                        ...
                        <p style=" margin-top: 6px; margin-bottom:0px;">{{\App\CPU\translate('invoice')}}: #{{ $order->id }}</p>
    ...
    <div class=" content-height content-position-y" style="">
        <table class="customers bs-0">
            ...
            <tr class="for-th" style=" border: 1px solid #2D7BFF;margin-top: 5px">
                <th class="for-th for-th-font-bold" style="color: black">{{\App\CPU\translate('no.')}}</th>
                <th class="for-th for-th-font-bold " style="color: black">{{\App\CPU\translate('item_description')}}</th>
            ...
            @php
                ...
                $previousTotal_shipping_cost=0;
                $total_discount_on_product=0;
                $previousTotal_discount_on_product=0;
                $ext_discount=0;
                $previous_ext_discount=0;
            @endphp
            <tbody>
                @foreach($previousOrder->details as $key=>$details)
                @php $previousSubtotal=($details['price'])*$details->qty+$details['tax']-$details['discount'] @endphp
                <tr class="for-tb" style=" border: 1px solid #2D7BFF;">
                    ...
                    <td class="for-tb "  style=" border: 1px solid #2D7BFF;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details['price']))}}</td>
                    <td class="for-tb"  style=" border: 1px solid #2D7BFF;">{{$details->qty}}</td>
                    <td class="for-tb"  style=" border: 1px solid #2D7BFF;">- {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($previousSubtotal))}} <small class="text-warning">RETURN</small></td>
                </tr>

                @php
                    $previousSubtotal+=$previousSubtotal;
                    $previousTotal_tax+=$details['tax'];
                    $previousTotal_shipping_cost+=$details->shipping ? $details->shipping->cost :0;
                    $previousTotal_discount_on_product+=$details['discount'];
                    $previousTotal+=$previousSubtotal;
                @endphp
            @endforeach
            @foreach($order->exchangedetails as $key=>$details)
                @php $subtotal=($details['price'])*$details->qty+$details['tax']-$details['discount'] @endphp
                <tr class="for-tb" style=" border: 1px solid #2D7BFF;">
                    <td class="for-tb "  style=" border: 1px solid #2D7BFF;">{{$key+1}}</td>
                    <td class="for-tb"  style=" border: 1px solid #2D7BFF;">
                        {{$details['product']?$details['product']->name:''}}
                    ...
                    <td class="for-tb "  style=" border: 1px solid #2D7BFF;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details['price']))}}</td>
                    <td class="for-tb"  style=" border: 1px solid #2D7BFF;">{{$details->qty}}</td>
                    <td class="for-tb"  style=" border: 1px solid #2D7BFF;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($subtotal))}}</td>
                </tr>
            ...
            <th style="text-align: right;width:60%;">
                <table style="width: 96%;margin-left:31%; display: inline " class="text-right sm-padding strong bs-0">
                    ...
                    <tr class="bg-primary" style="background-color: #2D7BFF">
                        ...
                        <td >
                           <b> {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->order_amount)-$order->advance_amount-$previousTotal)}}</b>
                        </td>
                    </tr>
                    </tbody>
                </table>
